...
public void BodyEat_OnPlayerKilled(SaxtonHaleBase boss, Event event, int iVictim)
{
  ...
  int iPlayerDamage = SaxtonHale_GetDamage(iVictim);
	int iHeal = RoundToNearest(float(iPlayerDamage)*0.4) + 50;
	...
	SDK_AlwaysTransmitEntity(iRagdoll); // doesn't seem to call
